...
export default function App() {
  ...
  async function handleSubmit() {
    setSyncing(true);
    const record = { ...form, gps: gps.coords, timestamp: Date.now(), farmer: "Koffi Mensah", coop: "COOPAC Kloto" };
    ...
    if (offline) {
      ...
      setSyncing(false);
      setStep(5); // success/queued screen
      return;
    }
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "flex-start", minHeight: "100vh", background: "#dedad3", padding: "20px 12px", fontFamily: "'Segoe UI','Helvetica Neue',sans-serif" }}>
      <style>{`
        ...
        .card { background:#fff; border-radius:14px; padding:16px 16px; margin-bottom:12px; }
        .row-field { display:flex; justify-content:space-between; align-items:center; padding:10px 0; border-bottom:1px solid #f0f0f0; font-size:13px; }
        .row-field:last-child { border-bottom:none; }
        .chip { display:inline-flex; align-items:center; gap:6px; padding:10px 14px; border-radius:10px; border:1.5px solid #e0e0e0; font-size:13px; cursor:pointer; transition:all .2s; }
      ...
      <div style={{ width: 375, background: GBG, borderRadius: 38, overflow: "hidden", boxShadow: "0 28px 72px rgba(0,0,0,.28)" }}>
        ...
        <div style={{ background: GD, padding: "11px 20px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {/* offline toggle chip */}
            <button onClick={() => setOffline(o => !o)} style={{ background: offline ? "rgba(232,153,10,.25)" : "rgba(255,255,255,.15)", border: "none", borderRadius: 8, padding: "2px 8px", cursor: "pointer", color: W, fontSize: 10, fontWeight: 600 }}>
        ...
        <div style={{ background: GD, padding: "8px 18px 16px", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            <div style={{ color: "rgba(255,255,255,.65)", fontSize: 11, marginBottom: 2 }}>Nouvelle récolte</div>
            <div style={{ color: W, fontSize: 19, fontWeight: 700 }}>Enregistrement blockchain</div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {queueCount > 0 && (
              <button onClick={drainQueue} style={{ position: "relative", background: "rgba(232,153,10,.25)", border: "1px solid rgba(232,153,10,.5)", borderRadius: 10, padding: "6px 10px", cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}>
                <Ico.Cloud synced={false} />
                <span style={{ color: W, fontSize: 11, fontWeight: 700 }}>{queueCount}</span>
              </button>
            )}
            ...
            <div style={{ width: 36, height: 36, borderRadius: 9, background: "rgba(255,255,255,.25)", display: "flex", alignItems: "center", justifyContent: "center", color: W, fontWeight: 700, fontSize: 13 }}>KM</div>
          </div>
        </div>
        ...
        <div style={{ background: GBG, padding: "16px 14px 0", minHeight: 520 }}>
          ...
          {step === 0 && (
            <div style={{ animation: "fadeUp .3s ease" }}>
              ...
              <div className="card">
                {SPECIES.map(sp => (
                  <div key={sp.id} onClick={() => setForm(f => ({ ...f, species: sp }))}
                    style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 10px", borderRadius: 10, cursor: "pointer", marginBottom: 4, background: form.species?.id === sp.id ? GL : "transparent", border: `1.5px solid ${form.species?.id === sp.id ? G : "transparent"}`, transition: "all .2s" }}>
                    <span style={{ fontSize: 22 }}>{sp.emoji}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: form.species?.id === sp.id ? 700 : 500, fontSize: 14, color: form.species?.id === sp.id ? G : "#222" }}>{sp.label}</div>
                      <div style={{ fontSize: 11, color: "#999" }}>Origine : {sp.origin}</div>
                    </div>
              ...
              <div className="card">
                <div style={{ textAlign: "center", padding: "10px 0 16px" }}>
                  <div style={{ position: "relative", display: "inline-flex", alignItems: "baseline", gap: 6 }}>
                    <input
                      ...
                      style={{ width: 130, fontSize: 52, fontWeight: 800, color: G, border: "none", background: "transparent", textAlign: "right", outline: "none", caretColor: G }}
                    />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6, marginTop: 6 }}>
                  {[50, 100, 200, 500].map(n => (
                    <button key={n} onClick={() => setForm(f => ({ ...f, weight: String(n) }))}
                      style={{ padding: "8px 0", borderRadius: 9, border: `1.5px solid ${form.weight == n ? G : "#e0e0e0"}`, background: form.weight == n ? GL : W, color: form.weight == n ? G : "#555", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
                      {n} kg
                    </button>
                  ))}
                </div>
                ...
                <div style={{ marginTop: 14, padding: "10px 12px", background: "#f8f8f6", borderRadius: 10, fontSize: 12, color: "#888", display: "flex", alignItems: "center", gap: 6 }}>
                  <span>ℹ</span>
                  Saisissez le poids brut incluant l'emballage.
                </div>
              </div>

              <div style={{ display: "flex", gap: 10 }}>
                <Btn outline onClick={() => setStep(0)} style={{ width: "auto", flex: 0, padding: "15px 20px" }}>← Retour</Btn>
                <Btn disabled={!canNext[1]} onClick={() => setStep(2)}>Suivant →</Btn>
              </div>
            </div>
          )}
            ...
            <div style={{ animation: "fadeUp .3s ease" }}>
              ...
              <div className="card">
                <input type="date" value={form.date} max={new Date().toISOString().slice(0, 10)}
                  ...
                  style={{ width: "100%", fontSize: 20, fontWeight: 700, color: G, border: "none", outline: "none", background: "transparent", cursor: "pointer", padding: "6px 0" }} />
                <div style={{ height: 1, background: "#eee", margin: "10px 0" }} />
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {[0, 1, 2, 7].map(d => {
                    const dt = new Date(); dt.setDate(dt.getDate() - d);
                    const iso = dt.toISOString().slice(0, 10);
                    const label = d === 0 ? "Aujourd'hui" : d === 1 ? "Hier" : `Il y a ${d}j`;
                    return (
                      <div key={d} className={`chip ${form.date === iso ? "active" : ""}`} onClick={() => setForm(f => ({ ...f, date: iso }))}>
                        {label}
                      </div>
                    );
                  })}
              ...
              <div className="card">
                ...
                {form.photoURL ? (
                  <div style={{ position: "relative" }}>
                    ...
                    <button onClick={() => setForm(f => ({ ...f, photo: null, photoURL: null }))}
                      style={{ position: "absolute", top: 8, right: 8, background: "rgba(0,0,0,.55)", border: "none", color: W, borderRadius: 8, padding: "4px 8px", cursor: "pointer", fontSize: 12 }}>
                      ✕ Supprimer
                    </button>
                    <div style={{ marginTop: 8, padding: "7px 10px", background: GL, borderRadius: 8, fontSize: 12, color: G, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
                      <Ico.Check s={16} /> Photo capturée
                    </div>
                  </div>
                ) : (
                  <button onClick={() => fileRef.current.click()}
                    style={{ width: "100%", height: 130, border: `2px dashed ${G}`, borderRadius: 12, background: GL, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, cursor: "pointer" }}>
                    <Ico.Camera s={32} />
                    <span style={{ fontSize: 14, fontWeight: 600, color: G }}>Prendre une photo</span>
                    <span style={{ fontSize: 11, color: "#888" }}>Ou importer depuis la galerie</span>
                  </button>
                )}
                <div style={{ marginTop: 10, fontSize: 11, color: "#aaa", textAlign: "center" }}>
                  La photo est optionnelle mais recommandée pour la traçabilité.
                </div>
              </div>

              <div style={{ display: "flex", gap: 10 }}>
                <Btn outline onClick={() => setStep(1)} style={{ width: "auto", flex: 0, padding: "15px 20px" }}>← Retour</Btn>
                <Btn disabled={!canNext[2]} onClick={() => setStep(3)}>Suivant →</Btn>
              </div>
            </div>
          )}

          {/* ─ STEP 3 : Validation + GPS ─ */}
          {step === 3 && (
            <div style={{ animation: "fadeUp .3s ease" }}>
              <SectionTitle icon={<Ico.Clipboard />} text="Récapitulatif" />
              <div className="card">
                {[
                  { l: "Espèce", v: form.species?.label },
                  { l: "Poids brut", v: `${form.weight} kg` },
                  { l: "Date de récolte", v: new Date(form.date).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" }) },
                  { l: "Producteur", v: "Koffi Mensah" },
                  { l: "Coopérative", v: "COOPAC Kloto" },
                  { l: "Photo", v: form.photoURL ? "✓ Capturée" : "Non fournie" },
                ].map(({ l, v }) => (
                  <div key={l} className="row-field">
                    <span style={{ color: "#888" }}>{l}</span>
                    <span style={{ fontWeight: 700, color: "#111", maxWidth: 180, textAlign: "right" }}>{v}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="card">
                {gps.status === "loading" && (
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", color: "#666", fontSize: 14 }}>
                    <Ico.Spinner />
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 15, color: "#111" }}>
                          {gps.coords.lat.toFixed(4)}° N,&nbsp;{gps.coords.lng.toFixed(4)}° E
                        </div>
                        <div style={{ fontSize: 11, color: "#999", marginTop: 2 }}>
                          Région des Plateaux — Kloto • Précision {gps.coords.acc}m
                        </div>
                      </div>
                    </div>
                    <button onClick={gps.capture} style={{ marginTop: 10, background: "none", border: `1px solid ${G}`, color: G, borderRadius: 8, padding: "5px 12px", fontSize: 11, cursor: "pointer", fontWeight: 600 }}>
                      ↺ Recapturer
                    </button>
                  </>
                )}
                {gps.status === "idle" && (
                  <Btn onClick={gps.capture} style={{ padding: "12px 0" }}>
                    <Ico.Pin s={16} col={W} /> Capturer ma position
                  </Btn>
                )}
              </div>

              {offline && (
                <div style={{ padding: "10px 14px", borderRadius: 10, background: "rgba(232,153,10,.1)", border: `1px solid rgba(232,153,10,.35)`, marginBottom: 12, fontSize: 12, color: "#8a5e00", display: "flex", gap: 8, alignItems: "flex-start" }}>
                  <Ico.Cloud synced={false} />
                  <span>Mode hors-ligne actif. L'enregistrement sera synchronisé automatiquement à la reconnexion.</span>
                </div>
              )}

              <div style={{ display: "flex", gap: 10, marginBottom: 4 }}>
                <Btn outline onClick={() => setStep(2)} style={{ width: "auto", flex: 0, padding: "15px 20px" }}>← Retour</Btn>
                <Btn disabled={!canNext[3]} loading={syncing} onClick={handleSubmit}>
                  {offline ? "💾 Enregistrer (offline)" : "Enregistrer sur la blockchain"}
                </Btn>
              </div>
            </div>
          )}
          ...
          {step === 5 && (
            <div style={{ animation: "fadeUp .4s ease", textAlign: "center", padding: "20px 0 10px" }}>
              ...
              <div style={{ fontWeight: 800, fontSize: 20, color: syncState.queued ? "#7a5500" : G, marginBottom: 6 }}>
                {syncState.queued ? "Enregistré hors-ligne" : "Blockchain confirmée !"}
              </div>
              <div style={{ fontSize: 13, color: "#888", marginBottom: 18, lineHeight: 1.6 }}>
                {syncState.queued
                  ? `L'enregistrement sera synchronisé automatiquement dès le retour du réseau. ${queueCount} récolte(s) en attente.`
                  : "Votre récolte a été inscrite de manière immuable sur la blockchain."}
              </div>

              {syncState.hash && (
                <div style={{ background: W, borderRadius: 12, padding: "12px 14px", textAlign: "left", marginBottom: 16 }}>
                  <div className="row-field">
                    <span style={{ color: "#888", fontSize: 12 }}>Hash blockchain</span>
                    <span style={{ fontFamily: "monospace", fontSize: 11, color: G, fontWeight: 700 }}>{syncState.hash}</span>
                  </div>
                  <div className="row-field">
                    <span style={{ color: "#888", fontSize: 12 }}>Block ID</span>
                    <span style={{ fontFamily: "monospace", fontSize: 11, color: "#333" }}>#{syncState.blockId}</span>
                  </div>
                  <div className="row-field">
                    <span style={{ color: "#888", fontSize: 12 }}>Espèce</span>
                    <span style={{ fontWeight: 700, fontSize: 12 }}>{form.species?.label}</span>
                  </div>
                  <div className="row-field">
                    <span style={{ color: "#888", fontSize: 12 }}>Poids</span>
                    <span style={{ fontWeight: 700, fontSize: 12 }}>{form.weight} kg</span>
                  </div>
                </div>
              )}

              {queueCount > 0 && !syncing && (
                <Btn onClick={drainQueue} style={{ background: OR, marginBottom: 10 }}>
                  ↑ Synchroniser {queueCount} récolte(s) en attente
                </Btn>
              )}
        ...
        <div style={{ background: W, borderTop: "1px solid #ebebeb", display: "flex", justifyContent: "space-around", alignItems: "center", padding: "10px 6px 18px" }}>
          {[
            { label: "Agriculteur", icon: <Ico.Farmer active={true} />, active: true },
            { label: "Coopérative", icon: <Ico.Coop /> },
            { plus: true },
            { label: "Vérificateur", icon: <Ico.Verif /> },
            { label: "Exportateur", icon: <Ico.Export /> },
          ].map((item, i) => item.plus ? (
            <div key={i} style={{ width: 48, height: 48, borderRadius: 13, background: G, display: "flex", alignItems: "center", justifyContent: "center", color: W, fontSize: 24, fontWeight: 300, cursor: "pointer", marginTop: -10, boxShadow: "0 4px 14px rgba(44,95,26,.35)" }}>+</div>
          ) : (
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer" }}>
              {item.icon}
              <span style={{ fontSize: 10, color: item.active ? G : "#aaa", fontWeight: item.active ? 700 : 400 }}>{item.label}</span>
            </div>
          ))}
        </div>
      </div>
